...
# Fresh build dir to avoid stale CMake cache when include paths change
mkdir -p build
cd build

# Resolve python (prefer an active venv)
PYTHON_BIN=${PYTHON_BIN:-"$(command -v python3)"}
# If project-local venv exists, prefer it
if [[ -f "$ROOT_DIR/.venv/bin/python" ]]; then
	PYTHON_BIN="$ROOT_DIR/.venv/bin/python"
fi
# Derive base prefix of the selected Python (the real install, not the venv)
PY_BASE_PREFIX=$("$PYTHON_BIN" -c "import sys; print(sys.base_prefix)")
echo "Python base_prefix: $PY_BASE_PREFIX"

# Derive include/lib from the selected Python to help CMake's FindPython
...
# Try to discover nanobind from Python first (pip install nanobind)
NB_CMAKE_DIR=""
NB_INCLUDE_DIR=""
...
# Fallback to common system prefixes (macOS/Homebrew, Linux /usr/local,/usr)
if [[ -z "${NB_CMAKE_DIR}" || -z "${NB_INCLUDE_DIR}" ]]; then
	for PFX in /opt/homebrew /usr/local /usr; do
		...
		# lib(cmake) layout seen on some Linux distros
...
# Last resort: use pkg-config to get include dir
...
# pybind11 is still discovered by MLIR (even if we use nanobind); provide cmake dir if available
PYBIND11_DIR=""
...
# Advise user to install robin-map when nanobind lacks vendored dependency
# (nanobind-config.cmake: find_dependency(tsl-robin-map))
if ! (
