...
`proof/` 51 モジュール 11575 行、`--safe`、postulate / TERMINATING / hole ゼロ、`make` / `make redcheck` 通過時点。

`GAP-ANALYSIS.md` は **モデルと C++ 実装の差** を扱う。この文書は **モデル内部で何が未証明か** を扱う。

---

## 0. 棚卸しの方法と、この会話で 8 回起きた計測失敗
...
どちらも計測器であり、**この会話で 8 回壊れた**。多くは「壊れた計測器が 0 / 合格を報告する」形、4 回目だけ逆向きだった:
...
| 6 | 全 golden の census が `build/bin/lyc` を毎回呼ぶ最中に、**私がそのコンパイラを再ビルドした** | 「296 件で重複 0」(測定対象が途中で変わった) |
| 7 | census の正規表現 `(%\S+)\s*:` が **複数オペランドのマーカー** `cast %9, %19 :` に一致しない | 「297 件で重複 0」— 実際は 3 件 |
| 8 | ソースを復旧して**リビルドせず** ctest を回した (6 番と同型、原因も同じく私) | 4 件の SKIP を「並行性の問題」と読みかけた |
...
- **計測中に計測対象を差し替えない。** 6 番の教訓。バイナリを凍結してから測る (`CENSUS_LYC`)。
- **ソースを戻したらビルドも戻す。** 8 番。`git diff --exit-code` が通っても、それはソースの話でバイナリの話ではない。
- **サンプル数を結論の強さに合わせる。** LSan を「`use_stacks=0:use_registers=0` で決定的」と書いたが、それは 8〜12 回の観測で、実際の沈黙率は 1/40 だった。数%のフレークを否定するには数十回要る。
- **既知陽性は代表的でなければ意味がない。** 7 番は既知陽性で検証**済み**だった — その陽性が単一オペランドのマーカーだったので、複数オペランド形を一度も踏まなかった。「反応した」ではなく「**測りたい母集団の全形を踏んだ**」を確認する。
- **正規表現の走査器より、コンパイラ自身に歩かせる。** 7 番の正しい母数 (502 件中 3 件) は、検査器をパイプラインに入れて ctest を回すことで出た。構造を歩く計測器は綴りを間違えられない。
...
### ⭐ `backed` はコンパイラ側で機械検査されるようになった (2026-07-30) — 精製の最初の 1 歩

`proof/` はコンパイラを一切拘束していない (下の F) が、**不変条件 1 本が初めて移った。**

`WFES.backed` (所有名は自分の site を占める = 1 値 1 所有トークン) を
`verifyOwnedTokenUniqueness` としてフェーズゲートに実装し、打刻する pass と消費する pass
の間に登録した。**ctest 504/504 で通る。**

これは精製 (refinement) ではない — モデルと IR の間に翻訳の証明は無く、対応は人が書いた
コメントである。しかし性質としては同じものが両側にあり、**コンパイラ側は機械が検査する**。
F が要求している「表現上の決定」のうち 1 つが実際に入った形である。

経緯も記録しておく価値がある。この検査器は登録した瞬間に**既知でなかったリーク 2 件**を
出した (`cross_exception_field_box_slot` 9 root、`sequence_literal_source_move_frequency`
3 root)。誰かが探しに行ったのではなく、**モデル由来の不変条件を機械に検査させたら落ちた**。
GAP-ANALYSIS の "read-back token" 行が「モデルが先に正解を出した」件で、これはその続きで
ある。

設計上の注意 2 点:

- **支配関係を見る。** 相互排他なブロックの 2 トークンは片方しか実行されないので正当。
  単なる共起で落とすと正しい IR を拒否する。
- **immortal は静的に判別できない。** モデルは `counted-exact` を `counted n` に限定して
  除外しているが、コンパイラ側の不変性は実行時の値 (refcount = INT64_MAX) である。現状
  該当ケースは無いので error で通るが、将来出たら免除ではなく打刻側の dedup で対処する。
...
### 17. ⭐ `dup` の実行時半分を切り出した — `Proof.Program.Recorded`

境界予測が外れたとき、理由はこう特定した: **モデルの `dup` は原子的**で、カウンタ増加・site 占有・名前束縛が構成上まとめて起きる。だから冗長な `dup` は `WFRC` も coherence も保存し、**モデルは余分な retain を「安全」だと言う**。コンパイラは `Ly_IncRef` 1 本でカウンタだけ動かせるので、実際の操作はどの規則よりも小さい。

閉じた。素のカウンタ操作を**機械上の関数**として定義し、それが不変条件を破ることを証明した:

| 定理 | 内容 |
|---|---|
| `retain-without-a-site-breaks-WFRC` | site を伴わない retain の後、`counted-exact` は `suc n ≡ n` を要求する |
| `release-without-vacating-breaks-WFRC` | 鏡像。`suc n` で述べてある — `bumpDown (counted zero)` は動かないので全カウンタに量化すると偽になる |

**規則にしなかった理由。** 規則にすると `step-preserves-WF` の全域性から切り出す必要がある (保存できないのがまさに内容なので)。不健全な操作 1 つのために全域性定理を弱めるより、操作に名前を付けて何が壊れるかを証明するほうが得る。`Proof.Program.Step` は**意味論が持つ操作の関係**であり続け、これは持たない操作である。

`Proof.Program.Run` に実物を置いた。`s₁` はカウンタ 1・site 1 で、`retainWithoutASite (mach s₁) theObj` は `¬ WFRC`。対になる `real-dup-is-well-formed` (= `midpoint-is-well-formed`) も並べてある — 片方だけでは間違ったことを証明してしまう。到達可能な機械が 1 つも `WFRC` を満たさないなら「ある機械が満たさない」は無意味だし、素の増加も保存するなら「`dup` が保存する」は無意味である。

**言っていないこと。** 不変オブジェクトのカウンタはカウントではない (`bumpUp immortal ≡ immortal`) ので、素の retain は何も壊さない。`counted-exact` はまさにそれを除外するために `counted n` で guard されており、上の 2 定理も `counted` 前提を取る (変異テストで load-bearing を確認)。この免除はコンパイラ側では静的に判定できないので、site 側を強制するフェーズゲート `verifyOwnedTokenUniqueness` は**これらの定理より厳しい** — immortal でも二重トークンを拒否する。IR としては不正なので方向は正しいが、**ゲートをこの定理だけで正当化はできない。**

変異テスト (arity 保存、両方向対照):

| 変異 | 結果 |
|---|---|
| コメントのみ（対照） | ACCEPTED |
| `retainWithoutASite` を no-op に | REJECTED |
| `releaseWithoutVacating` を no-op に | REJECTED |
| `counted` 前提を `o ≡ o` に弱める | REJECTED |
