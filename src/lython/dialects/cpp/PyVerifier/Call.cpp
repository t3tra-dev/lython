...
mlir::LogicalResult CallOp::verify() {
  ...
  if (FuncType funcTy = mlir::dyn_cast<FuncType>(callableType)) {
    signature = funcTy.getSignature();
  } else {
    return emitOpError("unexpected callable type");
  }

  TupleType tupleTy = mlir::dyn_cast<TupleType>(getPosargs().getType());
  if (!tupleTy)
  ...
  if (!signature.hasVararg()) {
    ...
    TupleType calleeVarargTy =
        mlir::dyn_cast<TupleType>(signature.getVarargType());
  ...
  mlir::Type kwargsType = getKwargs().getType();
  bool kwargsIsNone = mlir::isa<NoneType>(kwargsType);
  DictType dictTy = mlir::dyn_cast<DictType>(kwargsType);
...
mlir::LogicalResult CallVectorOp::verify() {
  mlir::ArrayAttr expectedArgNamesAttr;
  mlir::ArrayAttr expectedKwNamesAttr;
  mlir::FailureOr<FuncSignatureType> signatureOr = resolveCallableSignature(
  ...
  if (mlir::failed(verifyVectorCallOperands(
...
mlir::LogicalResult NativeCallOp::verify() {
  PrimFuncType primFuncTy = mlir::dyn_cast<PrimFuncType>(getCallee().getType());
  if (!primFuncTy)
    return emitOpError("callee must be of type !py.prim.func");

  mlir::FunctionType fnType = primFuncTy.getSignature();
