...
FailureOr<int> runJIT(ModuleOp module, const py::IRDumpConfig &irDump,
...

  {
    ...
    // The two knobs the exception tables depend on, from the one place that
    // decides them; the AOT path sets the same pair. ORC's default code model
    // is Large, which moves the LSDA type table to eight-byte entries and put
    // every JIT raise on x86-64 Linux into the personality's refuse block.
    tmBuilder.setRelocationModel(
        lython::driver::exceptionTableRelocationModel());
    tmBuilder.setCodeModel(lython::driver::exceptionTableCodeModel());
    tmBuilder.setCPU(
